...
- `og-the-loop.png` — the social preview card, built from the Loop
  ...
  up mostly margin.
- `sitemap.xml`, `robots.txt` — generated by `scripts/build-pages.py`.
  The sitemap lists only the indexable pages; the chapters are
  deliberately absent because they are `noindex`.
...
How they work — the prose is still rendered in the browser, but each
canonical URL now has a small HTML shell of its own:

- `scripts/build-pages.py` writes one shell per chapter slug
  (`chapters/<slug>.html`) and one per living page, so those URLs answer
  **HTTP 200**. The shell contains no prose: `chapters/reader.js` matches
  `/chapters/<slug>` against its manifest, fetches
  `chapters/md/<file>.md`, and renders it with
  [markdown-it](https://github.com/markdown-it/markdown-it) plus its
  footnote plugin (vendored in `chapters/vendor/`, no CDN at runtime).
  Re-run it after changing the manifest.
- `404.html` is still the fallback, and still earns its place: the 22
  canonical slugs have files, but the **46 aliases do not**, and they
  resolve through it exactly as before. It is also the genuine
  page-not-found. What changed is its status — it went from being how
  the site works to how old links and real errors are handled.
- **Chapters are `noindex`, living pages are not.** The chapter shells
  carry `<meta name="robots" content="noindex">`: crawled and read,
  never listed. `/template` and `/further-reading` are indexable,
  because the book prints those URLs and a reader has to be able to find
  them.
- **Do not add `Disallow: /chapters/` to `robots.txt`.** A page that
  cannot be crawled cannot have its `noindex` read either, which would
  leave the chapters eligible to appear as bare URLs — the opposite of
  the intent. `sitemap.xml` lists only the four indexable pages;
  noindexed URLs do not belong in a sitemap.
...
- The old tradeoff here is resolved. Every canonical URL used to answer
  404, so link unfurls showed no preview and search engines treated the
  pages as nonexistent. They answer 200 with a real `<title>` and
  `og:` tags now. Aliases still answer 404, which is the right level of
  care for a legacy URL.

To add a chapter: add one entry to the manifest in `reader.js` in
reading order, then run `python scripts/build-pages.py` to write its
shell. Forgetting the second step is not fatal — the chapter still
resolves through `404.html` — but it answers 404 and stays out of
unfurls until the file exists.
...
They resolve through the `PAGES` map in `reader.js` rather than the
chapter manifest, and each has its own shell from
`scripts/build-pages.py`, so they answer 200 and are indexable. Only
names in that map resolve at the top level; everything else falls
through to "page not found", so chapters do not quietly gain a second
URL at the root.
