...
There is a harder reading, and this book has to own it. The 2024 figures said AI adoption came with less throughput, not more, and if implementation had already become as abundant as the industry believes, the throughput line should have shown it. A year later it did: DORA's 2025 report found the relationship reversed, throughput and time on valuable work both turning positive.[^c8-dora] That is convenient for the premise this book runs on, which is exactly why it deserves the harder reading rather than a victory lap. What did not reverse is instability. The dividend arrived at the task level, and the system still cannot absorb it, and the difference is everything upstream of the agent: the thin spec, the wrong thing built quickly, the rework nobody counts. State it exactly, not conceded: a throughput gain a team cannot land safely is not a dividend it has cashed, and until the process lets it cash, the room costs principal. The test has a shape, and it is Appendix A's quarter: count gated specs, agent reruns, and rework tickets. If a quarter of full sessions moves none of those numbers, the premise was wrong for that team.
